@tool
@icon( "res://UI copy/Dialogue_system/icons/chat_bubble.svg" )
...
func _on_selection_changed() -> void:
	...
	if not sel.is_empty():
		if self == sel[0]:
			example_dialog = load("res://UI copy/Dialogue_system/dialogue_system.tscn").instantiate() as DialogSystemNode
			if example_dialog == null:
				return
